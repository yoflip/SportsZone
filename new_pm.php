...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
	{
		...
		if($dn1['recip']==1)
		{
			...
			{
				$id = $dn1['npm']+1;
				if(mysqli_query($con, 'insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['userid'].'", "'.$dn1['recipid'].'", "'.$message.'", "'.time().'", "yes", "no")'))
				{
	?>
