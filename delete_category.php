...
{
$id = intval($_GET['id']);
$dn1 = mysqli_fetch_array(mysqli_query($con,'select count(id) as nb1, name, position from categories where id="'.$id.'" group by id'));
if($dn1['nb1']>0)
{
...
{
	if(mysqli_query($con, 'delete from categories where id="'.$id.'"') and mysqli_query($con,'delete from topics where parent="'.$id.'"') and mysql_query('update categories set position=position-1 where position>"'.$dn1['position'].'"'))
	{
	?>
