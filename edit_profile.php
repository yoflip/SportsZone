		...
		if($_POST['password']==$_POST['passverif'])
		{
			if(strlen($_POST['password'])>=6)
			{
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					$dn = mysqli_fetch_array(mysqli_query($con, 'select count(*) as nb from users where username="'.$username.'"'));
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						if(mysqli_query($con, 'update users set username="'.$username.'", password="'.$password.'", email="'.$email.'", avatar="'.$avatar.'" where id="'.mysqli_real_escape_string($con, $_SESSION['userid']).'"'))
						{
							$form = false;
...
<?php
	...
	if($form)
	{
		...
		{
			$dnn = mysqli_fetch_array(mysqli_query($con, 'select username,email,avatar from users where username="'.$_SESSION['username'].'"'));
